 ...
 *                                if not set alwaysShow, when item has more than one children route, 如果没有设置alwaysShow，当item有多个子路由时，
 *                                it will becomes nested mode, otherwise not show the root menu  它将变成嵌套模式，否则不显示根菜单
 * redirect: noRedirect           if set noRedirect will no redirect in the breadcrumb
 * name:'router-name'             the name is used by <keep-alive> (must set!!!) 必须设置
 * meta : {
    roles: ['admin','editor']    control the page roles (you can set multiple roles)
    title: 'title'               the name show in sidebar and breadcrumb (recommend set)
    icon: 'svg-name'/'el-icon-x' the icon show in the sidebar
    noCache: true                如果设置为true，页面将不会被缓存(默认为false)
    affix: true                  如果设置为true，会固定显示在tag-view
